...
export function registerUserDORoutes(
  app: Hono<{ Bindings: Env; Variables: { user: User } }>,
  ...
  app.post('/api/signup', async (c): Promise<Response> => {
    try {
      ...
      const { user, token, refreshToken } = await userDO.signup({
...
export function getUserDOFromContext(
  c: Context,
  email: string,
  ...
  if (!binding) {
    throw new Error(
      `Durable Object binding '${bindingName}' not found. Make sure it's configured in wrangler.jsonc`
    );
  }
...
export function createUserDOWorker(bindingName: string = 'USERDO') {
  ...
  registerUserDORoutes(app, getUserDO);
  return app;
}

export { UserDO };

export const userDOWorker = createUserDOWorker();
